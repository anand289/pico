...
def check_humd_exceed_limits(humd):
    if humd < min_humd:
        lcd.clear()
        lcd.putstr("Humd too low!")
        lcd.putstr("\nLower than: "+str(min_humd))
        return True          
    elif humd > max_humd:
        lcd.clear()
        lcd.putstr("Humd too high!")
        lcd.putstr("\nHigher than: "+str(max_humd))
        return True
    else:
        return False
...
while True:
    ...
    if (t != t_prev) or (h !=h_prev):
        ...
        while check_humd_exceed_limits(d.humidity()):
            pass
